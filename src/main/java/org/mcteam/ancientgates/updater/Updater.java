...
public abstract class Updater {
	...
	protected Updater(final JavaPlugin plugin, final int id, final UpdateType type) {
		this.plugin = plugin;
		this.id = id;
		this.type = type;
				...
				if (this.versionCheck(this.versionName)) {
					if (this.versionLink != null && this.getType() == UpdateType.DOWNLOAD) {
						try {
							...
							Plugin.log(Level.WARNING, "The auto-updater tried to download a new update, but was unsuccessful.");
	...
	protected final boolean versionCheck(final String remoteVersion) {
		...
		try {
			...
			Plugin.log(Level.WARNING, "Error reading version number!");
		}
